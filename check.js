...
/*
function checkAll(){
  if(!checkEmail(User.inputEmail.value)){
    return false;
  }
  if(!checkPW(User.inputPW1.value,User.inputPW2.value)){
    return false;
  }
  if(!checkStudentID(User.inputStudentID.value)){
    return false;
  }
 if(!checkPhoneNum(User.inputPhoneNum.value)){
    return false;
  }
  return true;
}
...
function checkEmail(email){
  ...
  if(!regEmail.test(email)){
    ...
    User.inputEmail.focus();
    return false;
  }
  return true;
}
function checkPW(pw1,pw2){
  if(!checkExistData(pw1,"비밀번호를")) return false;
  if(!checkExistData(pw2,"비밀번호를")) return false;
...
function checkStudentID(id){
  ...
  if(ID.length < 9){
    alert('학번은 10자리입니다.');
    User.inputStudentID.value = "";
    User.inputStudentID.focus();
    return false;
  }
  return true;
}
function checkPhoneNum(num){
  ...
  var regNum = /01[016789]-[^0][0-9]{2,3}-[0-9]{3,4}/;
  if(!regNum.test(num)){
    alert('올바른 휴대폰 번호가 아닙니다.');
    User.inputPhoneNum.value = "";
    User.focus();
    return false;
  }
  return true;
